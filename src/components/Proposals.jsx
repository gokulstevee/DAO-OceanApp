...
const Proposals = () => {
  ...
  //getClosed() will return the proposals that are closed or duration expired
  const getClosed = () => {
    return setProposals(
      data.filter(
        (proposal) => new Date().getTime() > Number(proposal.duration + '000')
  ...
  //handlePayout() will execute the payment processing of sending ether
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {proposals.map((proposal) => (
                  <tr
                    key={proposal.id}
                    className="border-b dark:border-gray-500s"
                  >
                    ...
                    >
                      ...
                      {/* If proposal is expired and the proposal got maximum votes also if the proposer paid the beneficiary than it shows paid,
    if not yet paid than payout option will shown. But if got rejected with minimum votes than it shows rejected.
    If duration not yer expired and there is a time to still vote it shows no options.
    */}
                      {new Date().getTime() >
