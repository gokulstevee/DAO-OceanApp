...
const CreateProposal = () => {
  ...
  //handleSubmit() will handle the form
  const handleSubmit = async (e) => {
    ...
    toast.success('Proposal created, reloading...');
    closeModal();
  };

  // closeModal() will change the tailwind-css class from "scale-100" to "scale-0"
  const closeModal = () => {
    setGlobalState('createModal', 'scale-0');
    resetForm();
  };

  //resetForm() will erase the data from state of the form
  const resetForm = () => {
    setTitle('');
    setDescription('');
    setBeneficiary('');
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#212936] shadow-xl shadow-[#122643]
      ...
      >
        <form action="submit" className="flex flex-col">
          ...
          border border-gray-500 dark:border-gray-500 rounded-xl mt-5"
          >
            <input
              type="text"
              className="block w-full text-sm bg-transparent
            border-0 focus:outline-none focus:ring-0 p-2"
              name="title"
              placeholder="Title"
              onChange={(e) => {
          ...
          <div
            className="flex flex-row justify-between items-center border border-gray-500
          ...
          >
            <textarea
              className="block w-full text-sm resize-none bg-transparent 
              border-0 focus:outline-none focus:ring-0 h-20 p-2"
              type="text"
              name="description"
